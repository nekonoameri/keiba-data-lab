...
course=[dict(x) for x in con.execute('''SELECT r.track,r.surface,r.distance,COUNT(*) races,ROUND(AVG(r.trifecta_payout)) avg_trifecta,ROUND(100.0*AVG(CASE WHEN EXISTS(SELECT 1 FROM runners u WHERE u.race_id=r.race_id AND u.finish<=3 AND u.popularity>=7) THEN 1 ELSE 0 END),1) longshot_place_pct FROM races r GROUP BY r.track,r.surface,r.distance ORDER BY races DESC''')]
