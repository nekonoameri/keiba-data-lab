...
jockey=[dict(x) for x in con.execute('''SELECT u.jockey AS jockey,COUNT(*) rides,SUM(u.finish=1) wins,SUM(u.finish=2) seconds,SUM(u.finish=3) thirds,ROUND(100.0*SUM(u.finish=1)/COUNT(*),1) win_rate,ROUND(100.0*SUM(u.finish<=2)/COUNT(*),1) top2_rate,ROUND(100.0*SUM(u.finish<=3)/COUNT(*),1) place_rate,ROUND(AVG(u.popularity),1) avg_popularity,ROUND(100.0*SUM(CASE WHEN u.popularity>=7 AND u.finish<=3 THEN 1 ELSE 0 END)/NULLIF(SUM(CASE WHEN u.popularity>=7 THEN 1 ELSE 0 END),0),1) longshot_place_rate,ROUND(AVG(CASE WHEN u.popularity IS NOT NULL THEN u.popularity-u.finish END),2) popularity_gain FROM runners u JOIN races r ON r.race_id=u.race_id WHERE u.jockey<>'' AND u.jockey NOT IN ('柴田 善臣','柴田善臣','藤岡 佑介','藤岡佑介','和田 竜二','和田竜二') AND r.race_date>=date(strftime('%Y',?)||'-01-01') AND r.race_date<=? AND EXISTS (SELECT 1 FROM runners ua JOIN races ra ON ra.race_id=ua.race_id WHERE ua.jockey=u.jockey AND ra.race_date BETWEEN ? AND ?) GROUP BY u.jockey HAVING COUNT(*)>=10 ORDER BY wins DESC,seconds DESC,thirds DESC LIMIT 200''',(TODAY,TODAY,ACTIVE_SINCE,TODAY))]
