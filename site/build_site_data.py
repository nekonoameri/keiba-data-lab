...
jockey=[dict(x) for x in con.execute('''SELECT jockey,COUNT(*) rides,SUM(finish=1) wins,SUM(finish=2) seconds,SUM(finish=3) thirds,ROUND(100.0*SUM(finish=1)/COUNT(*),1) win_rate,ROUND(100.0*SUM(finish<=2)/COUNT(*),1) top2_rate,ROUND(100.0*SUM(finish<=3)/COUNT(*),1) place_rate,ROUND(AVG(popularity),1) avg_popularity,ROUND(100.0*AVG(CASE WHEN popularity>=7 AND finish<=3 THEN 1 ELSE 0 END),1) longshot_place_rate,ROUND(AVG(CASE WHEN popularity IS NOT NULL THEN popularity-finish END),2) popularity_gain FROM runners WHERE jockey<>'' GROUP BY jockey HAVING COUNT(*)>=30 ORDER BY place_rate DESC,rides DESC LIMIT 200''')]
