...
recent=[dict(x) for x in con.execute("""SELECT race_id,race_date,track,race_no,race_name,surface,distance,going,CASE WHEN trifecta_payout BETWEEN 100 AND ? THEN trifecta_payout ELSE NULL END AS trifecta_payout,source_url FROM races WHERE race_date IS NOT NULL AND race_date<=? AND (distance IS NULL OR distance BETWEEN 800 AND 4000) ORDER BY race_date DESC,race_no DESC LIMIT 120""",(MAX_TRIFECTA,TODAY))]
...
# rebuild-marker: active-allowlist-and-payout-guard-v4
