...
with open(root/a.races,encoding='utf-8-sig') as f:
    for x in csv.DictReader(f):
        if not valid_race_date(x['race_date']):
            print('SKIP invalid/future race date',x.get('race_id',''),x.get('race_date','')); continue
        con.execute('INSERT OR REPLACE INTO races VALUES (?,?,?,?,?,?,?,?,?,?,?)',(x['race_id'],x['race_date'],x['track'],val(x['race_no'],int),x['race_name'],x['surface'],val(x['distance'],int),x['going'],val(x['starters'],int),val(x['trifecta_payout'],int),x['source_url']))
